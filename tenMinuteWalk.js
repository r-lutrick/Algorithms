...
function isValidWalk(walk) {
    ...
    for (let i = 0; i < walk.length; i++) {
        m.hasOwnProperty(walk[i]) ?
            m[walk[i]] += 1 :
            m[walk[i]] = 1
    }
    return walk.length == 10 && walk.length % 2 == 0 && m['n'] == m['s'] && m['e'] == m['w']
}
console.log(isValidWalk(test1))
console.log(isValidWalk(test2))
console.log(isValidWalk(test3))
console.log(isValidWalk(test4))
