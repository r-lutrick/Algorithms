...
function musicRuntime(busDuration, songDurations) {
    hashmap = {}
    target = busDuration - 30
    for (let i = 0; i < songDurations.length; i++) {
        hashmap[songDurations[i]] = i
    }
    songDurations.sort((a, b) => { return b - a })
    for (let i = 0; i < songDurations.length; i++) {
        let complement = target - songDurations[i]
        if (complement in hashmap) {
            return [hashmap[songDurations[i]], hashmap[complement]]
        }
    }
    return [-1, -1]
}
